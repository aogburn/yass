...
# This script is preparing the local GIT repository for an update of the yass.sh. This includes:
# - updating the md5 file

# Usage: sh ./prepare-yass-update.sh

SCRIPT_SH="$(basename "$(test -L "$0" && readlink "$0" || echo "$0")")"
BASE_DIR=$(dirname "$(readlink -f "$0")")/../

YASS_SH="${BASE_DIR}/yass.sh"
MD5="${BASE_DIR}/md5"
